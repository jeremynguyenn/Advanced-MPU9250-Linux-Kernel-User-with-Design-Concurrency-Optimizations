...
#include <urcu/call_rcu.h> // New: call_rcu for deferred free
...
static void read_shared_seqlock(struct mpu9250_shared *shared, float *accel, float *gyro, float *mag, float *quat) {
    unsigned seq;
    int retries = 0;
    int max_retries = 100; // New: Retry limit to avoid livelock
    uint64_t old_ts = 0;
    do {
        ...
        old_ts = shared->timestamp; // New: Clock sync check
        if (read_seqretry(&shared->seq, seq)) {
            retries++;
            if (retries > max_retries) { // New: Livelock detection
                fprintf(stderr, "Livelock detected in seqlock retry\n");
                break;
            }
        }
    } while (read_seqretry(&shared->seq, seq) || old_ts == shared->timestamp); // New: Ensure clock sync
}
...
int main(int argc, char **argv)
{
    ...
    if (ioctl(fd, MPU9250_IOCTL_SETUP_EVENTFD, efd) < 0) {
